...
- Ensure work is on a **feature branch** (not the base branch); if the current branch is the base branch, create a feature branch first per `ai-specs/specs/git-workflow.mdc`.
- Create or update the PR for the current feature branch:
...
- **Do not merge** the PR unless the user **explicitly** asks you to merge after their approval. Default policy: the **maintainer must approve** the PR in GitHub before merge (`ai-specs/specs/git-workflow.mdc`).
- If the repo uses branch protection or required checks, mention that the PR is ready for **maintainer review** once checks pass.
...
- `ai-specs/.commands/develop-backend.md`: Use `gh` for GitHub and PR creation; feature branch + PR; maintainer approval before merge.
- `ai-specs/specs/git-workflow.mdc`: Feature branches and PR approval policy.
